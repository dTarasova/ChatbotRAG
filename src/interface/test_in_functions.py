...
# Sample answers from two different models (these could be dynamically generated)
def get_model_answers(question):
    ...
    answer1 = f"Answer from Model 1 for: {question}"
    answer2 = f"Answer from Model 2 for: {question}"
    return answerGPT, answerRAG


st.title("Compare OpenAI Models' Answers")

# Text input for the user to enter a question
question = st.text_input("Enter a question to compare answers:")

if question:
    # Get answers from the models (in practice, you'd call the models' API here)
    ...
    if st.button("Submit your choice"):
        ...
        if choice == "Answer 1 (Left)":
            user_choice = answers[0][0]
            selected_model = answers[0][1]
        else:
            user_choice = answers[1][0]
            selected_model = answers[1][1]
        
        # Log the choice to a file
        log_choice(question, answers[0][0], answers[1][0], user_choice, selected_model)
        st.success(f"Your choice has been logged! \n Your question : {question} You selected: {selected_model}")

    # Display the answers side by side
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Answer 1 (Left)")
        st.write(answers[0][0])
    with col2:
        st.subheader("Answer 2 (Right)")
        st.write(answers[1][0])
